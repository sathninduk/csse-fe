...
import {
    ...
    getSitesByDatetimeCount,
    getSitesByStatus,
    getSitesByStatusCount,
    getSitesCount, updateSitesStatus, updateSitesStatusBulk
} from "@/services/SitesManagementService";
...
export default function Sites() {
    ...
    const columns = [
        ...
        {name: "Seq_ID", uid: "seq_id", sortable: false, type: "text"},
        {name: "DOMAIN", uid: "domain", sortable: true, type: "text"},
    ...
    const updateStatusButton = (id, status) => {
            ...
            updateSitesStatus(id, status).then(() => {
    ...
    const updateStatusBulk = (ids, status) => {
        ...
        updateSitesStatusBulk(ids, status).then(() => {
            refreshData("success", "Updated");
        }).catch((error) => {
            headerMessage("error", error.response.data.error);
        });
    ...
    const handleUpdateStatusBulk = (selectedKeys, status) => {
        if (selectedKeys === 'all') { // if all items are selected
            ...
            updateStatusBulk(Array.from(selectedKeys).map((str) => parseInt(str, 10)), status);
        }
    };
    ...
    const refreshData = (type, message) => {
        ...
        getSitesCount(searchColumn, searchFieldValue).then((response) => setPagesCount(response));

        // fetch data from API // TODO: Change the following function
        getSites(rowsPerPage, currentPage, searchColumn, searchFieldValue)
    ...
    const fetchTableData = (useCallback((page, key, val) => {
        ...
        // fetch data count from API // TODO: Change the following function
        getSitesCount(key, val).then((response) => setPagesCount(response));

        // fetch data from API // TODO: Change the following function
        getSites(rowsPerPage, page, key, val)
            .then(response => setData(response === null ? [] : response.length === 0 ? [] : response))
            .catch(error => console.error(error));
    ...
    const onTimeRangeChange = (start, end) => {
        if (start === null || end === null) {
            ...
            getSitesByDatetimeCount(start, end, searchColumn, searchFieldValue).then((response) => setPagesCount(response));

            // get data // TODO: Change the following function
            getSitesByDatetime(rowsPerPage, currentPage, start, end, searchColumn, searchFieldValue).then(response => setData(response === null ? [] : response.length === 0 ? [] : response))
        }
    }
    ...
    const statusChange = (statusArray) => {
        // get data count // TODO: Change the following function
        getSitesByStatusCount(statusArray, searchColumn, searchFieldValue).then((response) => setPagesCount(response));

        // get data // TODO: Change the following function
        getSitesByStatus(rowsPerPage, currentPage, statusArray, searchColumn, searchFieldValue).then(response => setData(response === null ? [] : response.length === 0 ? [] : response))
    }
