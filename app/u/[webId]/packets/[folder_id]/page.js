...
export default function PinnedPackets({params}) {
    ...
    const refreshData = (type, message) => {
        ...
        getPinnedPacketsCount(searchColumn, searchFieldValue, params.webId, params.folder_id).then((response) => setPagesCount(response));

        // fetch data from API // TODO: Change the following function
        getPinnedPackets(rowsPerPage, currentPage, searchColumn, searchFieldValue, params.webId, params.folder_id)
    ...
    const fetchTableData = (useCallback((page, key, val) => {
        ...
        // fetch data count from API // TODO: Change the following function
        getPinnedPacketsCount(key, val, params.webId, params.folder_id).then((response) => setPagesCount(response));

        // fetch data from API // TODO: Change the following function
        getPinnedPackets(rowsPerPage, page, key, val, params.webId, params.folder_id)
            .then(response => setData(response === null ? [] : response.length === 0 ? [] : response))
            .catch(error => console.error(error));
    ...
    return (
        <>
            {contextHolder}
            <p className={"text-medium text-gray-200 mb-2"}>Pinned Data Packets</p>
            <Breadcrumbs className={"mb-8"}>
                <BreadcrumbItem><Link href={"../web/packets"}>Folders</Link></BreadcrumbItem>
